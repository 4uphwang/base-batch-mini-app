 ...
 */
export const useMiniappParams = (): { action: string | null, cardId: number | null } => {
    const searchParams = useSearchParams();
    // URL 파라미터가 변경될 때만 파싱 로직을 다시 실행하도록 useMemo 사용
    const parsedParams = useMemo(() => {
        const result: { action: string | null, cardId: number | null } = { action: null, cardId: null };
